      ...
      <footer className="bg-green-100 dark:bg-gray-900 text-green-900 dark:text-gray-200 py-8 px-4 mt-10">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
          ...
          <div>
            ...
            <p><button onClick={() => handleOpen('privacy')} className="hover:underline">Privacy Policy</button></p>
            <p><button onClick={() => handleOpen('terms')} className="hover:underline">Terms of Service</button></p>
            <p><button onClick={() => handleOpen('cookies')} className="hover:underline">Cookie Policy</button></p>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-center md:justify-start gap-4 text-2xl mt-2">
              <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="hover:text-green-700 dark:hover:text-green-300"><FaFacebook /></a>
              <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="hover:text-green-700 dark:hover:text-green-300"><FaTwitter /></a>
              <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="hover:text-green-700 dark:hover:text-green-300"><FaInstagram /></a>
            </div>
          </div>
        </div>
      ...
      {openModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 dark:bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 p-6 rounded-md max-w-lg shadow-lg relative">
            <h2 className="text-xl font-bold mb-4 capitalize">{openModal.replace(/^\w/, c => c.toUpperCase())}</h2>
            <p>{modalContent[openModal]}</p>
            <button onClick={handleClose} className="absolute top-2 right-2 text-gray-600 dark:text-gray-300 hover:text-red-500 text-lg">✕</button>
            <div className="text-right mt-4">
              <button onClick={handleClose} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded">Close</button>
            </div>
          </div>
        </div>
      )}
